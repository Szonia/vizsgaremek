...
const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  ...
  try {
    ...
    return res.status(201).json({ message: "Regisztráció sikeres!", uid: userRecord.uid });
  } catch (error: any) {
    console.error(" Hiba a regisztrációnál:", error);
    return res.status(500).json({ error: error.message || "Hiba történt a regisztráció során." });
  }
};
